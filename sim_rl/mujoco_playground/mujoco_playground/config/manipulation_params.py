...
def brax_ppo_config(
  ...
  if env_name == "AlohaHandOver":
    ...
    rl_config.network_factory = config_dict.create(
      ...
      "AeroCubeGraspHW6Force",
      "AeroCubeGraspHW6ForceP",
      "AeroCubeGraspHW6ForceV",
  ):
    rl_config.num_timesteps = 300_000_000
    rl_config.num_evals = 10
    rl_config.num_minibatches = 32
  ...
  elif env_name == "AeroCubeGraspHW6ForceVT":
    # VT: 触觉引入后观测维度不变（17D state），但信号丰富度更高。
    # 从零训练，初始 lr=3e-4（非 fine-tune）。
    rl_config.num_timesteps = 300_000_000
    rl_config.num_evals = 10
    rl_config.num_minibatches = 32
    rl_config.unroll_length = 40
